...
HermesDeck 是 Hermes Agent 的浏览器控制台：多会话聊天、Agents、Models 配置概览、配置编辑、能力面板、受控终端与可安装 PWA。Deck 的运行时 source of truth 是 Hermes Agent API Server；Deck 不把 Hermes 的本地数据库、CLI 或本地 catalog 当作生产回退路径。
...
- **RBAC fail-closed**：Deck 有自己的登录 cookie、用户/角色和 Agent assignment。Hermes Agent 本身不是 Deck 多用户/RBAC 系统；多用户边界属于 Deck BFF。每条 Agent-scoped route 都必须先由 Deck server-side RBAC 授权；普通用户不得访问未分配 Agent/default。敏感 upstream 数据缺少 routing proof 时失败关闭；profileless session rows 只有在 Deck server-side 已证明 explicit identity、非 default 专用 Agent API base 或专用 API key 时才可按该 Agent stamped。profileless cron jobs 只有在响应确认 profile 或 Deck 证明 server-owned dedicated named-Agent routing 时才 stamped；共享/default route 失败关闭。
- **MFA**：Deck auth 支持 TOTP 2FA 与 passkey/WebAuthn 作为密码后的并列第二因子；不提供 passwordless 登录。TOTP enrollment 显示 QR code，并保留 manual secret/URI fallback；passkey 依赖浏览器安全上下文与 WebAuthn RP origin/id。`/api/deck/auth/mfa` 在 proxy 层是 public，因为登录二阶段发生在 session cookie 之前；route 本身按 action 校验 purpose-bound `mfaToken` 或受保护 session；TOTP 与 passkey verify 共用失败限速语义。
...
- **聊天流**：Deck BFF 先 POST Hermes API Server `/v1/runs`，再读取 `/v1/runs/{run_id}/events` SSE，向浏览器转发文本、raw run-event、attachment、done/error，并发送 keep-alive 注释保持长连接活性。当前 `/v1/runs` image attachment 发送为已有 attachment-annotated text prompt，不使用 `/v1/responses` 的 `input_image` content parts。工具调用不再有右侧运行事件小窗；扳手按钮打开时，聊天窗口主体会通用显示 `tool.started`/`tool.completed` 的 `payload.tool` 名称（如 `lcm_grep`、`hindsight_recall`），不做插件专用 UI。其它带 `run_id` 的非 delta Agent API events 也会作为隐藏的 event rows 保存在主 transcript，打开工具详情时按“具体事件名 → Run event”展示。聊天窗口工具卡片把具体工具名放在通用标签前；`delegate_task` 的后台派发 ack 与异步完成回填分别显示为 subagent dispatched/result。前端发送 35 分钟 timeout（2,100,000ms），服务端夹在 `[1000, 2100000]`，匹配 Hermes active subagent 30 分钟上限 + 5 分钟收尾余量。
...
- **Models 配置概览**：`/models` 仅在浏览器调用同源 `GET /api/deck/model-config?profile=<id>`；Deck BFF 做 auth/RBAC 后从 Hermes Dashboard 的 `/api/model/info`、`/api/model/auxiliary`、`/api/config`（只取 delegation 白名单）和 `/api/cron/jobs` 汇总只读 DTO。Dashboard 默认必须在 `http://127.0.0.1:9119` 可达；可用 server-only `HERMES_DASHBOARD_BASE` 覆盖。`HERMES_DASHBOARD_SESSION_TOKEN` 仅限 loopback Dashboard；remote gated Dashboard 不受此 BFF 支持。
- **Deck-owned chat projection**：`~/.hermesdeck/chat-projection.v1.json`（或 `HERMESDECK_DATA_DIR`）只保存 Deck UX/proof 状态，用 lock、atomic write、TTL/cap prune 维护；它不是 Hermes runtime 数据源。Projection 会持久化 draft/final assistant、tool-call、tool-result、非 delta `run-event` 行和 response/session aliases（不逐 delta 持久化 tool/function arguments），刷新后仍可显示 in-flight 状态；attachment 只保留 id/name/mime/size/kind metadata，不保存 pasted text/base64 data URL/raw URL。Completed sessions 默认保留 90 天。Completed message hydrate 优先使用 Hermes Agent canonical history 中的 `tool_calls` / `role='tool'` rows，只叠加 Deck-owned approval/run-event overlay；Agent history 不可用或不完整时才回退到 projection。Projection proof/write 对普通用户按 `ownerUserId` 收紧，admin/super_admin 仍需先通过 Agent 授权；shared/default API base 上的 unlabeled upstream session rows 仍 fail closed。
...
MFA 在 Settings 中启用：TOTP 可用任意 authenticator app 扫描 QR code，或手动录入 secret/`otpauth://` URI；passkey registration 只需要当前密码/受保护 session，不要求先启用 TOTP。Passkey 需要 `http://localhost` 或 HTTPS 域名；不要用 `127.0.0.1` / LAN IP。公网/反代部署 passkey 时请固定单个 `HERMESDECK_WEBAUTHN_ORIGIN` 与匹配的 `HERMESDECK_WEBAUTHN_RP_ID`，例如 `https://hermesdeck.beliefanx.me` / `hermesdeck.beliefanx.me`。
...
export HERMESDECK_PUBLIC_ORIGIN=https://hermesdeck.beliefanx.me
