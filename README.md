...
- **聊天流**：Deck BFF 先 POST Hermes API Server `/v1/runs`，再读取 `/v1/runs/{run_id}/events` SSE，向浏览器转发文本、raw run-event、attachment、done/error，并发送 keep-alive 注释保持长连接活性。工具调用不再有右侧运行事件小窗；扳手按钮打开时，聊天窗口主体会通用显示 `tool.started`/`tool.completed` 的 `payload.tool` 名称（如 `lcm_grep`、`hindsight_recall`），不做插件专用 UI。其它带 `run_id` 的非 delta Agent API events 也会作为隐藏的 `run-event` tool rows 保存在主 transcript，打开工具详情时以 `Agent API event` 卡片展示。聊天窗口工具卡片把具体工具名放在通用标签前；`delegate_task` 的后台派发 ack 与异步完成回填分别显示为 subagent dispatched/result。前端发送 35 分钟 timeout（2,100,000ms），服务端夹在 `[1000, 2100000]`，匹配 Hermes active subagent 30 分钟上限 + 5 分钟收尾余量。
