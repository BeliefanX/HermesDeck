...
- **MFA**：Deck auth 支持 TOTP 2FA 与 passkey/WebAuthn 作为密码后的并列第二因子；不提供 passwordless 登录。TOTP enrollment 显示 QR code，并保留 manual secret/URI fallback；passkey 依赖浏览器安全上下文与 WebAuthn RP origin/id。
