...
- **MFA**：Deck auth 支持 TOTP 2FA 与 passkey/WebAuthn 作为密码后的并列第二因子；不提供 passwordless 登录。TOTP enrollment 显示 QR code，并保留 manual secret/URI fallback；passkey 依赖浏览器安全上下文与 WebAuthn RP origin/id。`/api/deck/auth/mfa` 在 proxy 层是 public，因为登录二阶段发生在 session cookie 之前；route 本身按 action 校验 purpose-bound `mfaToken` 或受保护 session。
...
- **Deck-owned chat projection**：`~/.hermesdeck/chat-projection.v1.json`（或 `HERMESDECK_DATA_DIR`）只保存 Deck UX/proof 状态，用 lock、atomic write、TTL/cap prune 维护；它不是 Hermes runtime 数据源。Projection 会持久化 draft/final assistant、tool-call、tool-result、非 delta `run-event` 行和 response/session aliases（不逐 delta 持久化 tool/function arguments），刷新后仍可显示 in-flight 状态；completed sessions 默认保留 90 天。Completed message hydrate 优先使用 Hermes Agent canonical history 中的 `tool_calls` / `role='tool'` rows，只叠加 Deck-owned approval/run-event overlay；Agent history 不可用或不完整时才回退到 projection。Projection proof/write 对普通用户按 `ownerUserId` 收紧，admin/super_admin 仍需先通过 Agent 授权；shared/default API base 上的 unlabeled upstream session rows 仍 fail closed。
