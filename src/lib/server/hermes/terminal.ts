...
const apiTerminalActions: TerminalActionSpec[] = [
  ...
  { id: 'hermes.api.profiles', label: 'List Agents', description: 'List assignable Agents from the Hermes Agent API-backed catalog.', commandPreview: 'Hermes Agent API Agent catalog', category: 'hermes', maxTimeoutMs: 8000, build: () => ({ synthetic: async () => ({ stdout: JSON.stringify({ profiles: await getStrictProfiles() }, null, 2) }) }) },
];
