...
export default function ProfilesPage() {
  const t = useT({
    zh: {
      introA: 'Agent 是 HermesDeck 分配给账号的运行目标。每个 Agent 由独立的 Hermes Agent profile 支撑，状态目录为 ',
      introB: '。在下方选择一个 Agent，即可查看它实际使用过的服务商与模型。',
      kickerProfiles: '已分配 Agents',
      titleProfiles: 'Agents',
      kickerRouting: '路由 · 已用模型',
      titleRouting: '路由与模型',
      activeProfile: '当前 Agent',
      modelsHint: '此处只展示该 Agent 实际使用过的服务商与模型。配置中的默认项会被高亮；仅存在于配置目录、从未被调用的条目会被隐藏。',
      kickerProvidersUsed: '服务商 · 已使用',
      kickerModelsUsed: '模型 · 该 Agent 已用',
      ...
      baseUrl: 'base_url · ',
      loadFailed: '加载失败：',
      noProfiles: '未找到任何 Agent',
      noProfilesDesc1: 'HermesDeck 将以默认 Agent 运行。使用 ',
      noProfilesDesc2: ' 添加新的 Agent。',
      noModelUsage: '尚无模型使用记录',
      noModelUsageDesc: '该 Agent 还没有任何会话记录。开启一次聊天，即可在此查看服务商与模型的使用拆分。',
      runtimeRunning: '运行中',
      runtimeStarting: '启动中',
      runtimeStopped: '已停止',
      runtimeFailed: '已失败',
      runtimeUnknown: '未知',
      ariaSelectProfile: '选择 Agent',
    ...
    en: {
      introA: 'An Agent is the runtime target HermesDeck assigns to an account. Each Agent is backed by a Hermes Agent profile with its own state directory at ',
      introB: '. Pick an Agent below to see which providers and models it has actually used.',
      kickerProfiles: 'ASSIGNED AGENTS',
      titleProfiles: 'Agents',
      kickerRouting: 'ROUTING · USED MODELS',
      titleRouting: 'Routing & models',
      activeProfile: 'active Agent',
      modelsHint: 'Showing only providers and models that this Agent has actually used. The configured default is highlighted; catalog-only entries (configured but never invoked) are hidden.',
      kickerProvidersUsed: 'PROVIDERS · USED',
      kickerModelsUsed: 'MODELS · USED BY AGENT',
      ...
      noProfiles: 'No Agents found',
      noProfilesDesc1: 'HermesDeck will run with the default Agent. Use ',
      noProfilesDesc2: ' to add a new Agent.',
      noModelUsage: 'No model usage yet',
      noModelUsageDesc: 'This Agent hasn’t recorded any sessions yet. Start a chat to see provider and model breakdowns here.',
      runtimeRunning: 'Running',
      runtimeStarting: 'Starting',
      runtimeStopped: 'Stopped',
      runtimeFailed: 'Failed',
      runtimeUnknown: 'Unknown',
      ariaSelectProfile: 'Select Agent',
