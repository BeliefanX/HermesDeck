...
import { isLocalOnlyTerminalAction, runTerminalAction } from '@/lib/server/hermes';
import { guardMutating, guardRequestBody, readLimitedJson } from '@/lib/server/csrf';
import type { TerminalRunRequest } from '@/lib/types';
import { requireAdmin, requireSuperAdmin } from '@/lib/server/rbac';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const auth = isLocalOnlyTerminalAction(parsed.value.actionId) ? requireSuperAdmin(req) : requireAdmin(req);
