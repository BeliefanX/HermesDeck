...
- `ChatView.jsx` — sessions panel + thread with inline tool cards, plus the slash-command-aware composer. Current product no longer has a right-side run-events timeline rail; tool events use the existing inline card format (`<toolName> Tool call/result`), and raw non-delta lifecycle events appear only in the main transcript when tool details are enabled as `<eventName> Run event`.
...
- ⚠️ Tools / Settings — prototype coverage is partial and may lag current source. Check `src/app/*` for current product behavior; there are no current standalone `/profiles` or `/models` pages.
