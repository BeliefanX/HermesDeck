...
- `GET /api/deck/health`：Deck 与 Hermes API Server 健康状态、版本/uptime 等。当前详细 readiness 只暴露既有低敏 health 字段；auth/data/projection/VAPID/publicOrigin 的 admin-only 分项 readiness 尚未实现。
...
- `GET /api/deck/model-config?profile=<id>`：`/models` 的只读同源 BFF。鉴权/RBAC 后并行读取 Dashboard `/api/model/info`、`/api/model/auxiliary`、`/api/config`（仅 delegation 白名单）与 `/api/cron/jobs`；浏览器不会直接访问 Dashboard，返回 URL 仅保留 http(s) origin/path（去 userinfo/query/hash）。部分上游失败仍为 `200` 且响应带 `errors`；四项都失败为 `502`。`HERMES_DASHBOARD_SESSION_TOKEN` 仅限 loopback Dashboard；remote gated Dashboard 不受此 BFF 支持。
...
- `GET /api/deck/sessions/[id]/messages?profile=<id>`：返回 session messages；projection 可返回刷新后仍存在的 draft assistant、tool-call、tool-result rows。若 projected draft 的 upstream hydrate 因 alias / canonical session proof 暂不可证而返回 `session_profile_mismatch`，BFF 保留 viewer-scoped projection 响应，不把 live/in-flight thread 打成 403；无 projection 或读取他人 projection 仍 fail closed。completed Deck projection 会尝试读取 Hermes Agent canonical history；当 canonical messages 含 assistant `tool_calls` 或 `role='tool'` result rows 时，响应使用 canonical tool details，并仅叠加 Deck-owned approval/run-event overlay rows，projection 仍作为 Agent history 不可用/不完整时的 fallback。
