...
- 读取时会 normalize/repair 可接受旧 shape，但不会把 projection 当作 Hermes runtime 真相；completed session 若 Hermes Agent canonical history 已有 `tool_calls` / `role='tool'` rows，message hydrate 以 canonical rows 为准，并只叠加 projection-owned approval/run-event rows。
...
- `COMPLETED_SESSION_TTL_MS = 90 days`
...
- `onRunEvent`：raw run-events 仍转发给浏览器，并由聊天窗口主体里的工具卡片通用渲染 `tool.started`/`tool.completed` 的 `payload.tool` 名称；右侧不再有运行事件小窗。projection 在语义边界（tool/function call added、arguments done、output item done、tool result/output）物化为 messages；其它带 `run_id` 的非 delta Agent API run events 也以隐藏的 `role='tool'` / `projectionKind='run-event'` 记录保存，工具详情开启时在主 transcript 展示。`arguments.delta` / text delta 不做 durable projection write，刷新后的持久参数以 `arguments.done`/done item 为准。它会关联 Responses `itemId`（如 `fc_*`）和稳定 `callId`（如 `call_*`），将工具输出数组中的 text parts 归一化为文本。
...
- Completed projection hydrate 会先查 Hermes Agent canonical session messages；当 canonical history 已有 assistant `tool_calls` 或 `role='tool'` result rows 时，Deck 返回 canonical tool-call/result rows，并只叠加 projection-owned approval/run-event debug rows。若 Agent history 不可用或尚不完整，则继续返回 viewer-scoped projection fallback。90-day completed projection retention 仍用于这些 Deck-owned proof/debug rows。
