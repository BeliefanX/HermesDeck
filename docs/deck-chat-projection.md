...
Deck chat projection 是 HermesDeck 自有的 UX/proof store。它记录 Deck 通过 API Server 发起或观察到的聊天 turn，用于刷新后的会话列表、stream 状态、server-projected draft assistant/tool rows、named-profile session/response proof、错误状态展示。
...
- `onRunEvent`：raw run-events 仍转发给浏览器；projection 只在语义边界（tool/function call added、arguments done、output item done、tool result/output）物化为 messages。`arguments.delta` 不做 durable projection write，刷新后的持久参数以 `arguments.done`/done item 为准。它会关联 Responses `itemId`（如 `fc_*`）和稳定 `callId`（如 `call_*`），将工具输出数组中的 text parts 归一化为文本。
...
- `hasProjectedSession(sessionId, profileId, viewer)` 证明 session 属于 profile，且普通用户的 viewer 必须匹配 `ownerUserId`。
- `projectedResponseIdMatches(sessionId, profileId, previousResponseId, viewer)` 证明 response chain 属于该 profile session，且普通用户的 viewer 必须匹配 `ownerUserId`。
...
Projection 带 `ownerUserId/ownerRole`。Stream Hub 和 routes 仍以 Deck session cookie/RBAC 为准；普通用户只能读取、proof、continue、写入 `ownerUserId` 等于自己 Deck user id 的 projected sessions/messages/stats，且仍必须有对应 profile assignment。Admin/super_admin 可显式绕过 projection owner filter/proof/write check，但仍受 route-level profile auth/catalog 约束；catalog 仍必须来自 API Server。
