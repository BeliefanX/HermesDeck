...
- `src/lib/server/hermes/dashboard.ts`：`/models` 的 Dashboard-only 只读 adapter；浏览器经 `/api/deck/model-config` 同源 BFF 访问，不能直连 Dashboard。
...
- stats/messages/tokens/tools/lcm 等：通过 BFF 对应 adapter 暴露给 UI。Token analytics 当前是 Agent API 缺口 placeholder：返回 unavailable zeroed stats，不读取本地 runtime DB 聚合。LCM SQLite dashboard 是 `super_admin/local-owner` 本机管理面。
...
MFA：TOTP 2FA 与 passkey/WebAuthn 都是密码后的并列第二因子；没有 passwordless 登录。已启用 MFA 的用户在密码正确后只得到短期 password-MFA token，TOTP 或 passkey 验证成功后才写正式 `hermesdeck_session`。因此 `/api/deck/auth/mfa` 在 proxy 层 intentionally public，但 route 只让 login actions 使用 purpose-bound `mfaToken`，enrollment/settings actions 仍要求受保护 session。TOTP setup 显示 QR code，并保留 manual secret/URI fallback；passkey registration 需要 current password/受保护 session，但不要求 TOTP。TOTP 与 passkey login verify 都按 user id + client IP 记录失败并在成功后 reset；WebAuthn registration/login challenge 是 5 分钟进程内状态并按 purpose 隔离，不能跨 enrollment/login/MFA token 混用。
...
Projection 是 UX/proof 状态，不是 Hermes runtime 数据源。它保存 Deck 观察到的 sessions/messages、owner/Agent runtime id/status、response id aliases，用于刷新后会话列表、server-projected draft assistant 行、tool-call/tool-result 行、非 delta `run-event` 调试行、named-Agent proof、错误/完成状态展示；completed sessions 当前保留 90 天。Attachment projection 只保存安全 metadata（id/name/mime/size/kind），不持久化 pasted text、base64 data URLs 或 raw/private URLs。读取 completed projected messages 时，若 Hermes Agent canonical history 已有 assistant `tool_calls` 或 `role='tool'` result rows，messages route 使用 canonical tool details，并只叠加 projection-owned approval/run-event rows；Agent history 不可用或不完整时才回退到 projection。Hermes 异步 delegation 完成 marker 只在 Deck 展示层归一化为 assistant-side `delegate_task` subagent result，不写成 `role='tool'`。普通用户只可读取/证明/继续/写入 ownerUserId 等于自己的 projected sessions/messages/stats；admin/super_admin 绕过 owner filter/check，但仍需通过 Agent RBAC/catalog 授权。Deck 只在 server-side 已证明 explicit identity、专用 named-Agent API base 或专用 API key 时 stamp profileless upstream session rows；shared/default base+key 且 `/health` 无 identity 仍 fail closed。
...
`public/sw.js` 是 cache version 的来源；当前为 `hermesdeck-pwa-v61`：

- shell cache 只预缓存 `/offline`、manifest、icons。
- `/api/*` 网络直通；只有网络异常时合成 `{ ok:false, offline:true, error:'offline' }` 的 503。
- `/api/deck/cache-image` 不使用 SW cache，并清理旧命中；route 本身要求 `super_admin` 且只服务 image extensions/MIME（SVG 下载），避免 local cache artifact 泄露给普通用户。
