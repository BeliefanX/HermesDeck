...
- sessions/stats 等 sensitive upstream rows 必须由 API 响应 metadata 或 Deck server-owned 专用 Agent API base 证明 scope；shared/default API base 上的 unlabeled rows 返回 `profile_routing_unavailable`。cron/jobs 仍需要 API 响应证明。
...
Projection 是 UX/proof 状态，不是 Hermes runtime 数据源。它保存 Deck 观察到的 sessions/messages、owner/Agent runtime id/status、response id aliases，用于刷新后会话列表、server-projected draft assistant 行、tool-call/tool-result 行、named-Agent proof、错误/完成状态展示。普通用户只可读取/证明/继续/写入 ownerUserId 等于自己的 projected sessions/messages/stats；admin/super_admin 绕过 owner filter/check，但仍需通过 Agent RBAC/catalog 授权。Deck 只在 server-side 已路由到专用 named-Agent API base 时 stamp profileless upstream session rows；shared/default API base 仍 fail closed。
