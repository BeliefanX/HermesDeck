...
5. BFF 仍原样转发 raw `run-event`；浏览器右侧观测面板会通用显示 `tool.started`/`tool.completed` 的 `payload.tool` 名称（如 `lcm_grep`、`hindsight_recall`）。`onRunEvent` projection hook 只在 tool/function call/result 语义边界物化为 `tool-call`/`tool-result` message rows；tool output arrays 会归一化为文本；后台 `delegate_task` 派发 ack 与异步完成回填复用 subagent tool-result 卡片。
