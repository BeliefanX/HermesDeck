...
HermesDeck 是 Hermes Agent 的 Deck/BFF/UI 层。它在同一个 Next.js 进程中提供 UI 与 `/api/deck/*` BFF，并把运行时请求转发给 Hermes Agent API Server。Deck 的职责是鉴权、RBAC、profile scope、UI 投影、SSE 转发和安全缓存；Hermes runtime 的事实来源仍是 API Server。
...
- `src/lib/server/auth.ts` 与 `src/lib/server/rbac.ts`：Deck auth store、cookie session、角色能力、profile assignment 与 fail-closed 检查。
...
- profiles：`/v1/profiles`、`/api/profiles`，选择返回内容最完整的 API-backed catalog。
- models：按 profile 调 `/v1/models`，不从本地文件合成模型清单。
- chat：按 profile 调 `/v1/responses`，必要时传 `X-Hermes-Session-Id`。
- cron：按 profile 调 `/api/jobs?include_disabled=true&profile=<id>`，必须从响应或 job rows 得到 profile routing proof。
- runs/stats/messages/tokens/tools/lcm/kanban 等：通过 BFF 对应 adapter 暴露给 UI。

Deck 可以编辑 Hermes profile 文件（`config.yaml`、`SOUL.md`、`memories/USER.md`、`memories/MEMORY.md`），但这些编辑器不是运行时数据源。文档中不要把 Hermes 本地数据库、CLI 或本地 catalog 描述为生产路径。
...
- `admin`：可管理普通用户、使用 terminal、查看全部 API-backed profiles。
- `user`：只能访问被分配的 profiles。

Fail-closed 规则：

- 未登录、inactive、cookie 无效：401/403。
- 非 admin 缺少 profile assignment：403。
- profile id 无效：400。
- admin/super_admin 需要 catalog 时，如果 Hermes API Server catalog outage，返回错误/空降级提示，不枚举本地 profile 目录补齐。
- cron profile routing 未被 API 响应证明时返回 `profile_routing_unavailable`，不展示可能属于其他 profile 的 jobs。

## Chat/SSE 数据流

1. 浏览器向 `POST /api/deck/chat/stream` 发送 message、profile、model/reasoning override、attachments、可选 session/previous response。
